...
```
NG: ...CONST                     入力項目配列  ← 「入」が80桁目 → SO がEBCDIC 80桁
                                  ↑ col 80

OK: ...CONST                      入力項目配列 ← 「入」が81桁目 → SO がEBCDIC 81桁
                                   ↑ col 81
```

**ASCII始まりの混合注記も同様です。**
`Item obj番号` のような注記でも、先頭の `I` が 80 桁目にあると
...
```
CRTRPGMOD MODULE(SAMPLELIB/name)
...
```
NG: ...CONST                     項目数         ← 80桁目に「項」
OK: ...CONST                      項目数        ← 81桁目に「項」
```
...
```
NG: ...CONST                      入力データポインタ      ← 10文字=22バイト
OK: ...CONST                      入力データptr           ← 7文字+3ASCII=19バイト
```
