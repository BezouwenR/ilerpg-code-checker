...
- リポジトリ整理（チェッカーの検出ロジックに変更なし）
  - サンプル用ディレクトリ名のタイプミスを修正（`saples/` → `samples/`）
  - テストスクリプト3本（test-samples / test-dspec-overflow / test-dspf-overlap）を `tests/` に集約。単一ソース検証ヘルパー `tests/check-one.mjs <path>` を追加
- サンプルソースをサニタイズ＋圧縮（合計約4660行→約1500行）
  - 実題材を想起させないよう中立化し、`SAMPLE01`〜`SAMPLE05` にリネーム。アルゴリズム本体はダミー化、識別子・コメントも汎用化
  - 桁位置チェックの価値である**RPG構文の多様性は温存**し、全5本が**コンパイル相当（error 0件）**であることを維持
  - 同梱ドキュメントの proprietary 参照もサニタイズ（汎用RPG/EBCDIC知識は保持）

### 0.0.14 (2026-06-12)
- DSPF_FIELD_OVERLAP に**先頭属性バイトの衝突**検出を追加（実例: AITNTCFGD.dspf CTLWIN 行4 — `'状態:'`(4,2)=7バイト col2-8 と `'テナント'`(4,9) はデータ自体は隣接で重ならないのに CPD7866 が出た）
  - 5250では各フィールドの開始位置の**直前1桁を先頭属性バイトが占有**するため、前フィールドのデータ末尾と次フィールドのデータ開始の間に最低1桁の空きが必要。ギャップ0（end+1 から開始）は属性バイトが前フィールドのデータに乗り CPD7866 になる。判定を `next.col <= current.endCol` から `next.col <= current.endCol + 1` に拡張し、属性バイト衝突の場合は専用メッセージで報告
  - ギャップ1（end+2 から開始）は属性バイトが空き桁に収まるため正常（誤検出しない）
- DBCSHelper の**半角カタカナ誤分類を修正**: U+FF00-FFEF を一括で DBCS 扱いしていたため、半角カタカナ（U+FF61-FF9F、EBCDIC 5026/5035 ではコードページ290の SBCS=1バイト）のバイト長を2倍に過大計上していた。属性バイト判定の追加で `'日次ﾄｰｸﾝ上限 (0=無制限):'`（AIOPSD.dspf）が偽陽性化したことで発覚。半角形（U+FF61-FFDC, U+FFE8-FFEE）を SBCS に修正
- 回帰テストに5ケース追加（DBCS定数の属性バイト衝突/ギャップ1正常/SBCS定数直後のフィールド開始/同修正後/半角カタカナ含み定数）。`E:/IBM_i_and_AI/src` 全DSPF 10ファイルで偽陽性0件を検証。なお検証中に編集中の AITNTCFGD CTLWIN の `CWOPT`(14,12) を真陽性として検出（`'選択 ==>'` の `>` col11 と属性バイト衝突）

### 0.0.13 (2026-06-11)
- DSPF_FIELD_OVERLAP（CPD7866相当）の検出漏れを解消（実例: AIADMND.dspf TENFTR の無条件定数 `'F3/12=戻る  F5=再表示'` と標識77付き定数 `'F6=追加'` の重なりが未検出だった）
  - **定数（リテラル）を重なり判定の対象に追加**: 従来は名前付きフィールドのみ対象で、定数同士・定数とフィールドの重なり（CPD7866の典型パターン）が全て素通りだった。表示長はEBCDIC変換後のバイト長（DBCS n文字 = SO/SI込みで 2n+2 バイト）で確定計算。`'+'`（次行の最初の非ブランクから継続）/`'-'`（次行の桁45から継続）の定数継続行にも対応
  - **標識除外を「両方標識付きの場合のみ」に変更**: 従来は片方でも標識付きだと判定をスキップしていたが、CPD7866は「条件づけの指定のない別のフィールド」との重なり。77/N77 のような排他表示同士（CPD7865相当・意図的）のみ除外し、片方が無条件なら報告する
- DBCSHelper に CJK記号・句読点（U+3000-303F: 全角スペース、「」、。・等）を DBCS として追加（従来はバイト長を過小評価していた）
- 回帰テスト `test-dspf-overlap.mjs` を追加（修正前AIADMNDの重なり検出/修正後0件/排他表示の非検出/継続リテラル長）。`E:/IBM_i_and_AI/src` 全DSPF 10ファイルで偽陽性0件、全RPGソースで検出結果の変化なしを検証
